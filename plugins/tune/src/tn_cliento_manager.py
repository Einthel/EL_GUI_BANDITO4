...
class TuneClientoManager(QObject):
    ...
    def send_button_press(self, button_id):
        ...
        if self.socket_client:
            # Используем общий протокол Core (как shortcut):
            # command: PLUGIN_BUTTON_PRESS, payload: {"id": "<button_id>"}
            try:
                self.socket_client.send_command("PLUGIN_BUTTON_PRESS", {"id": button_id})
            except AttributeError:
                # Фоллбек для старых версий socket_client
                data = {
                    "command": "PLUGIN_BUTTON_PRESS",
